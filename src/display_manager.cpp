...
DisplayManager::DisplayManager(sf::RenderTarget &target,
                               sf::RenderWindow &window)
    : m_window(window), event_manager(window), m_target(target), m_zoom(1.0f),
      m_offsetX(0.0f), m_offsetY(0.0f), speed_mode(false), m_va(sf::Quads, 0),
      update(true), debug_mode(false), clic(false),
      m_mouse_button_pressed(false), pause(false), draw_markers(true) {
  ...
  event_manager.addEventCallback(
      ...
      [&](sfev::CstEv ev) { zoom(1 + ev.mouseWheel.delta * 0.2f); });

  event_manager.addEventCallback(
      sf::Event::MouseButtonPressed, [&](sfev::CstEv ev) {
  ...
  event_manager.addEventCallback(
      sf::Event::MouseButtonReleased, [&](sfev::CstEv ev) {
...
sf::Vector2f
DisplayManager::worldCoordToDisplayCoord(const sf::Vector2f &worldCoord) const {
...
sf::Vector2f
DisplayManager::displayCoordToWorldCoord(const sf::Vector2f &viewCoord) const {
