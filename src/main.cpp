...
float length(const sf::Vector2f &v) { return sqrt(v.x * v.x + v.y * v.y); }

sf::Vector2f normalize(const sf::Vector2f &v) { return v / length(v); }
...
struct Ball {
  ...
  Ball() : position_history(max_history), current_idx(0) {}

  Ball(float x, float y, float arg_r)
      : position(x, y), velocity(static_cast<float>(rand() % 8 - 4),
                                 static_cast<float>(rand() % 8 - 4)),
        r(arg_r), position_history(max_history), current_idx(0) {
  ...
  sf::VertexArray getVA() const {
    ...
    for (uint32_t i(0); i < max_history; ++i) {
      ...
      va[i].color = sf::Color(static_cast<sf::Uint8>(color.x),
...
bool update(std::vector<Ball> &balls, float speed) {
  ...
  for (uint32_t i(0); i < nBalls; i++) {
    ...
    for (uint32_t k = i + 1; k < nBalls; k++) {
      ...
      const sf::Vector2f collide_vec =
          current_ball.position - collider.position;
      const float dist =
          sqrt(collide_vec.x * collide_vec.x + collide_vec.y * collide_vec.y);
...
void updatePos(std::vector<Ball> &balls, float speedDownFactor,
...
int main() {
  ...
  sf::RenderWindow window(sf::VideoMode(conf.win_width, conf.win_height),
                          "NoCol", sf::Style::Fullscreen, settings);
  ...
  for (int i(0); i < nBalls; i++) {
    ...
    balls.push_back(Ball(start_x + WIN_WIDTH * 0.5f,
                         start_y + WIN_HEIGHT * 0.5f,
                         RNGf::getRange(static_cast<float>(minSize),
  ...
  display_manager.event_manager.addKeyReleasedCallback(
      sf::Keyboard::A, [&](sfev::CstEv) { drawTraces = !drawTraces; });
  display_manager.event_manager.addKeyReleasedCallback(
      sf::Keyboard::C, [&](sfev::CstEv) { traces.clear(); });
  display_manager.event_manager.addKeyReleasedCallback(
      sf::Keyboard::Space, [&](sfev::CstEv) {
        speedDownFactorGoal = speedDownFactor == 1 ? 10.0f : 1.0f;
      });
  display_manager.event_manager.addKeyReleasedCallback(
      sf::Keyboard::Escape, [&](sfev::CstEv) { window.close(); });
  display_manager.event_manager.addKeyReleasedCallback(
      sf::Keyboard::E, [&](sfev::CstEv) {
  ...
  while (window.isOpen()) {
    ...
    for (const Ball &b : balls) {
      ...
      const float stable_ratio =
          (ok_count > 199) ? 1.0f : std::min(1.0f, b.stableCount / 255.0f);
      const sf::Vector3f vec_color =
          stable_ratio * stable_color + (1.0f - stable_ratio) * unstable_color;

      int c = b.stableCount > 255 ? 255 : b.stableCount;
      sf::Color color(static_cast<sf::Uint8>(vec_color.x),
                      static_cast<sf::Uint8>(vec_color.y),
                      static_cast<sf::Uint8>(vec_color.z));
      float r = b.r;
